...
from algs.dwpi import (ENCODINGS, getWeightVecs,
	...
	encodeDemos, inferWeight, printDWPIResults)
...
def main():
	...
	# BIPI METHOD:	use new bayesean inference approach on the test demos saved to disk by testGen.py
		...
		# load the shared test set: true weights + demos generated by per-user policies (not DWPI's discretized grid)
		testData = loadTestData(envName)
		testWs, testDemos = testData['ws'], testData['demos']
		# use the beta the demos were actually generated with so rho matches the demonstrator model
		...
		for user in range(len(testWs)):
			prefWeight = testWs[user]
			demos      = testDemos[user]
			print(f"\nUser {user}: true preference weight = {np.round(prefWeight, 4)}")
			print(f"  {len(demos)} demo(s), {sum(len(d) for d in demos)} total steps")

			if renderBool:
				for i, demo in enumerate(demos):
					renderTrajectory(demo, f"demo {i}", envName)
	...
	# DWPI METHOD:	use new baseline inference approach on saved demo data
		...
		# step 5:	evaluate on the shared test demonstrators saved to disk by testGen.py
		# these demos come from per-user policies, not DWPI's discretized weight grid, so DWPI gets no home-field advantage
		obsToIdx = lambda obs: obsToStateIdx(obs, env)
		testData = loadTestData(envName)
		testWs, testDemos = testData['ws'], testData['demos']
		print(f"Loaded {len(testWs)} test demonstrators from disk")
	...
	# COMPARE METHOD: lunches comapritive analysis of bipi and dwpi, using saved dwpi model and ccs policies on disk
		...
		ccsDir  = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ccsResults', envName)
		saveDir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dwpiResults', envName)
		runCompare(env, envName, ccsDir, saveDir)
